...
### 3-1. 표 재포맷 + 연결/별도 명시 태그 + 2-shot 예시 (실험 13)

[docs/experiments.md](experiments.md) 실험 13에서 실측으로 검증한 3가지를 `_build_context()`/`FEWSHOT_MESSAGES`에 반영했다:

- **표 재포맷**(`_reformat_table_text`): 원시 `매출액 | 333,605,938 | 300,870,903` 대신 `매출액: 제57기=333,605,938, 제56기=300,870,903`처럼 열(기간)을 행마다 명시한다 - `chunk_type == "table"`일 때만 적용, 저장된 원본 텍스트는 안 건드리고 생성 컨텍스트를 만들 때만 변환한다.
- **연결/별도 명시 태그**(`_statement_basis_label`): breadcrumb에 `[연결기준]`/`[별도기준]`을 명시적으로 붙인다 - DART 관행상 "연결"이 안 붙은 "재무제표"가 별도재무제표를 뜻하는데, 이 관례를 3B 모델이 항상 아는 게 아니라서 명시가 필요했다(실측으로 확인).
- **2-shot 예시**(`FEWSHOT_MESSAGES`): (1) 비슷한 값 여러 개 중 정확한 계정과목+최근 기간을 고르는 예시, (2) 연결/별도가 둘 다 있을 때 연결을 고르는 예시.

**주의 - 표 재포맷을 처음 넣었을 때 오히려 정확도가 떨어졌었다**(53.3%→43.3%): 원시 표를 통일된 포맷으로 바꾸면서, 연결재무제표와 별도재무제표 두 표의 (약하지만 존재했던) 구조적 차이까지 같이 지워버려서 모델이 둘을 더 못 구분하게 됐다. 연결/별도 태그를 명시적으로 추가하고 나서야 60.0%로 개선됐다 - "모델이 읽기 쉽게 정리"하는 개선이 항상 안전한 건 아니라는 교훈.
